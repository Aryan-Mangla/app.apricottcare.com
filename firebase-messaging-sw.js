...
function showSystemNotification(title, body, data) {
  const tag = data.event || data.type || 'apricott-care-' + Date.now()
  const options = {
    body,
...
// Single display path for data-only messages (fires when the app is backgrounded,
// minimized, or all tabs are closed). Renders the image via showNotification.
messaging.onBackgroundMessage((payload) => {
  const data = payload.data || {}
  const title = data.title || 'Apricott Care'
  ...
  event.waitUntil(
    clients.matchAll({ type: 'window', includeUncontrolled: true }).then((clientList) => {
      for (const client of clientList) {
        if ('focus' in client) {
          client.focus()
          client.postMessage({ type: 'apricott:fcm-click', data })
